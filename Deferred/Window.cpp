...
Window::Window() 	{
	...
	if (shouldBeZero != 0) {
		std::cout << "Window.cpp:23: " << "Couldn't get display mode for video display" << std::endl;
	}
}
...
int Window::create(string windowName, int screenWidth, int screenHeight, unsigned int currentFlags) {
	...
	if (glContext == nullptr) {
		std::cout << "Window.cpp:66: " << "SDL_GL context could not be created" << std::endl;
	}
	...
	if (error != GLEW_OK) {
		std::cout << "Window.cpp:75: " << "Could not initialize glew!!" << std::endl;
	}
	...
	glClearColor(243.0f/255.0f, 184.0f/255.0f, 108.0f/255.0f, 1.0f);
